...
const resetCalculator = () => {
  ...
  // Reset UI
  document
    .querySelectorAll(".calculator-step")
    .forEach((step) => step.classList.remove("active"));
  document
    .querySelector('.calculator-step[data-step="1"]')
...
const goToStep = (stepNumber) => {
  ...
  document
    .querySelector(
      `.calculator-step[data-step="${calculatorState.currentStep}"]`,
    )
    .classList.remove("active");

  // Show new step
  document
    .querySelector(`.calculator-step[data-step="${stepNumber}"]`)
    .classList.add("active");
...
const updateProgressBar = () => {
  const progress =
    ((calculatorState.currentStep - 1) / (calculatorState.totalSteps - 1)) *
    100;
...
document.querySelectorAll(".standing-card").forEach((card) => {
  card.addEventListener("click", () => {
    document
      .querySelectorAll(".standing-card")
      .forEach((c) => c.classList.remove("selected"));
    card.classList.add("selected");
...
document.querySelectorAll('input[name="service"]').forEach((checkbox) => {
  checkbox.addEventListener("change", () => {
    calculatorState.services = Array.from(
...
document.querySelectorAll(".finish-level-card").forEach((card) => {
  card.addEventListener("click", () => {
    document
      .querySelectorAll(".finish-level-card")
      .forEach((c) => c.classList.remove("selected"));
    card.classList.add("selected");
...
const submitLeadForm = async () => {
  ...
  try {
    ...
    const response = await fetch(
      "https://formsubmit.co/ajax/ibe@ibe-construction.com",
      {
        method: "POST",
        body: formData,
        headers: {
